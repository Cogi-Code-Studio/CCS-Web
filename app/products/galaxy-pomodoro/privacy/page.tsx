...
const galaxyPomodoro = getProductBySlug("galaxy-pomodoro", "en");
const metadataImage = galaxyPomodoro
  ? getProductMetadataImage(galaxyPomodoro)
  : getAbsoluteUrl("/opengraph-image.png");
const metadataImageAlt = galaxyPomodoro
  ? getProductMetadataImageAlt(galaxyPomodoro)
  : "Cogi Code Studio pixel logo on a dark navy background.";

export const metadata: Metadata = {
  title: "Galaxy Pomodoro Privacy Policy",
  description: "Public privacy policy for the Galaxy Pomodoro Apple app.",
  keywords: [
    "Galaxy Pomodoro privacy policy",
    "Galaxy Pomodoro",
    "은하 뽀모도로 개인정보 처리방침",
    "pomodoro timer app privacy",
  ],
  alternates: {
    canonical: galaxyPomodoroPrivacyPath,
  },
  openGraph: {
    title: `Galaxy Pomodoro Privacy Policy | ${siteName}`,
    description: "Public privacy policy for the Galaxy Pomodoro Apple app.",
    type: "website",
  ...
  twitter: {
    card: galaxyPomodoro?.iconSrc ? "summary" : "summary_large_image",
    title: `Galaxy Pomodoro Privacy Policy | ${siteName}`,
    description: "Public privacy policy for the Galaxy Pomodoro Apple app.",
    images: [metadataImage],
  },
  ...(galaxyPomodoro?.iconSrc
    ? {
        icons: {
          icon: [
            {
              url: galaxyPomodoro.iconSrc,
              type: "image/png",
              sizes: "1024x1024",
            },
          ],
...
export default async function GalaxyPomodoroPrivacyPage() {
  const locale = await getCurrentLocale();
  const copy = siteCopy[locale];
  const policy = getGalaxyPomodoroPrivacy(locale);
  ...
  return (
    <>
      ...
      <main className="relative z-10 mx-auto flex w-full max-w-6xl flex-1 flex-col gap-10 px-6 pb-20 sm:px-8 lg:px-10">
        <Link className="pixel-chip w-fit" href="/products/galaxy-pomodoro">
